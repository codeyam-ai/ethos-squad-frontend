...
const MySquad: React.FC = () => {
    ...
    const squadNfts = useMemo(() => {
        return wallet?.contents?.nfts?.filter((nft) => nft.packageObjectId === TESTNET_NFT_CONTRACT)
                ...
                <div className="mx-auto max-w-2xl gap-x-14 lg:mx-0 lg:flex lg:max-w-none">
                    ...
                    <div className="w-full mt-14 flex justify-end gap-8 sm:-mt-44 sm:justify-start sm:pl-20 lg:mt-0 lg:pl-0">
                        <ul role="list" className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:gap-x-8">
                            {
                                status !== EthosConnectStatus.Connected ? <p className="text-gray-500">Connect your wallet to see your Squad NFTs!</p> :
                                    !squadNfts ? <p className="text-gray-500">Loading...</p> :
                                        squadNfts.length === 0 ? <p className="text-gray-500 text-center w-full">You don't have any Squad NFTs yet!</p> :
                                            squadNfts.map((nft) => {
                                                console.log('nft :>> ', nft);
                                                return (
